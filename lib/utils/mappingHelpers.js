...
/**
 * Developer links on Play use a query param `id` (often the developer name slug).
 * @param {string} [link] Relative path or absolute URL
 * @returns {string} Developer id or empty string if missing
 */
function extractDeveloperId(link) {
  if (link == null || typeof link !== 'string') return '';
  const tail = link.split('?id=')[1];
  if (tail == null) return '';
