...
function parseServiceRequests(response) {
  ...
  const literal = valueMatch[0];
  try {
    // Play embeds a JS object literal (not always JSON). Avoid eval in global scope:
    // run in an isolated context with a short timeout (still unsafe on untrusted HTML).
    return vm.runInNewContext(`(${literal})`, Object.create(null), {
      timeout: 100,
      displayErrors: true,
    });
  } catch (err) {
    debug('parseServiceRequests failed: %s', err.message);
    return {};
  }
}
