...
/**
 * Section titles for the similar carousel (vary by storefront language).
 * When none match, we fall back to the first cluster batch (best-effort).
 */
const SIMILAR_CLUSTER_TITLES = new Set([
  'Similar apps',
  'Similar games',
  'Ähnliche Apps',
  'Ähnliche Spiele',
...
function parseSimilarApps(similarObject, opts) {
  const clusters = scriptData.extractDataWithServiceRequestId(
    similarObject,
    INITIAL_MAPPINGS.clusters
  );
  ...
  let similarAppsCluster = clusters.filter((cluster) => {
    const title = R.path(CLUSTER_MAPPING.title, cluster);
    return typeof title === 'string' && SIMILAR_CLUSTER_TITLES.has(title);
  });
  ...
  return request(options, opts.throttle)
    .then(scriptData.parse)
    .then((htmlParsed) =>
      processFirstPage(htmlParsed, opts, [], INITIAL_MAPPINGS)
    );
}
...
async function processFirstPage(html, opts, savedApps, mappings) {
  ...
  const mapping = {
    ...
    price: {
      path: [8, 1, 0, 0],
      fun: (price) =>
        typeof price === 'number' && !Number.isNaN(price) ? price / 1000000 : 0,
    },
    free: {
      path: [8, 1, 0, 0],
      fun: (price) => price === 0,
