...
function processShortPermissionsData(html) {
  ...
  const validPermissions = commonPermissions.filter(
    (permission) => permission.length
  );
  const permissionNames = R.chain(
    (permission) => permission[MAPPINGS.type],
    validPermissions
  );
...
function processPermissionData(html) {
  ...
  const permissions = Object.values(constants.permission).reduce(
    (permissionAccummulator, permission) => {
      if (!html[permission]) {
