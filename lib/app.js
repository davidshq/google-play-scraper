...
/**
 * @param {{ appId: string, lang?: string, country?: string, throttle?: number, requestOptions?: object }} opts
 * @returns {Promise<object>} Parsed app fields (see tests for shape).
 * Options are shallow-cloned before applying `lang` / `country` defaults so the caller's object is not mutated.
 * `requestOptions` is deep-cloned so nested objects are not shared with the caller.
 */
function app(opts) {
  return new Promise(function (resolve, reject) {
    ...
    if (opts.requestOptions != null) {
      opts.requestOptions = structuredClone(opts.requestOptions);
    }
